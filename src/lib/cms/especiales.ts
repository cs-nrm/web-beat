 ...
 *
 * Se ordena por `-inicio` y se toma el primero, en vez de filtrar por
 ...
 *
 * Se conserva `piezas` como respaldo y en ese orden: el campo dedicado gana,
 * porque es el que el formulario presenta y el que el editor cree estar llenando.
 *
 * Devuelve `null` si llegó como id: a `depth: 1` —el de `obtenerEspeciales`—
 ...
 *
 * NO lleva `catch`: quien llama tiene que poder distinguir «no existe» de «el
