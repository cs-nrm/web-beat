...
**`src/layouts/Base.astro` lo comparten cuatro agentes** —metadata, ads,
...
**`pnpm check` da 0 errores en cosas que el compilador del build sí rechaza**
...
3. **El panel del navegador miente con las animaciones.** Reporta
...
4. **La cascada CSS se prueba sobre un build SERVIDO, no sobre `astro dev`**:
...
5. **`src/js/` es LEGADO.** `ads.js`, `analytics.js`, `player.js` y `votes.js`
...
6. **Los diseños nuevos no están en `design/`.** Los artboards llegan hasta v14
...
7. **Una zona sin contenido se deja vacía y sin rótulo.** Nada de «pronto habrá
...
8. **Un atributo sin valor vale la cadena vacía**, y `!!''` es `false`. Para
...
| Servicio | **systemd** `web-beat-v2`. No es pm2 |
...
Pero el nombre sigue vivo donde no es texto y **no se toca a la ligera**:
...
- **El porqué va en el código.** Los comentarios señalan lo que no se debe romper
  y las trampas medidas. **Léelos antes de cambiar la línea que
...
- `[skip deploy]` en el asunto del commit salta la puerta y el despliegue. No
