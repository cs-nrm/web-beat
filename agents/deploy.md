...
**No es pm2.** Se dio `pm2 restart` por error y no existe en esta máquina.
...
`--ff-only` a propósito: si la VM divergió, que falle. Un merge fabricado en el
...
**`main` sigue siendo el sitio viejo y no se toca.** Ya no sirve a nadie —el
...
El paso de SSH **no hace `git pull`** antes del script, a diferencia de los
...
`[skip deploy]` en el asunto del commit salta puerta y despliegue. No lo
...
- **`beatdigital.mx` era un `ServerAlias` del vhost de `oyedigital.mx`** en
  ...
  Antes de dar por bueno cualquier corte por vhost: `apachectl -S | grep -i
  <dominio>`, que enseña los alias. `apachectl configtest` **no** avisa de un
  nombre duplicado entre vhosts.
- **`PUBLICIDAD_TOKEN` sigue sin ponerse**: sin ella las campañas vendidas no
  ...
  punta.
- **`PUBLIC_METRICOOL_HASH` es de BUILD** y no entró en el artefacto del corte:
  ponerlo ahora obliga a reconstruir.
- **`scripts/cambiar-dominio.sh` tiene tres defectos reproducidos y ninguno
...
**`pnpm check` NO es una puerta suficiente.** El compilador del build rechaza
cosas que `astro check` acepta —un comentario mal cerrado dio 0 errores en `check`
y reventó el build—. La puerta es `pnpm build`.

Y la cascada se prueba sobre un **build servido**, no sobre `astro dev`:
