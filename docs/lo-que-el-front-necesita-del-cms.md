...
Convención: ✅ = verificado contra el CMS o la API. Lo demás está inferido y
...
Salió calcada de lo pedido, incluido el detalle crítico:
...
- **`suscripciones`** — el newsletter del §6. Decisión abierta: quién ENVÍA
...
Y que el sitemap sea un **índice con hijos paginados**, no un `<urlset>` plano:
...
Lo único que hace falta acordar es **cómo se van a llamar**, porque el slug ES la
...
## 5. Campos que el diseño pide y hay que confirmar antes de crear
...
- ❌ **La CLASIFICACIÓN de un programa — NO VA. Decisión de Carlos, 2026-09-09:**
  ...
  **Y no hay camino B.** `programas` no tiene NINGUNA taxonomía con la que
  ...
  Del lado del front se quitó todo —las casillas, la sección, el conteo y
...
- ✅ **Imagen del hero: UNA sola, con punto focal — RESUELTO el 2026-09-09.** No se
  ...
  **Los `sizes` del CMS son solo de ANCHO y conservan la proporción: ninguno es
  ...
  se quedaría con dos casillas sin regla de cuál gana. Esto sigue vigente aunque
...
- **`programas.estado` NO lo filtra el servidor** (avisado por la sesión del CMS,
  ...
  Lo que **no** filtra es `obtenerPrograma(slug)`, y es a propósito: la ayuda del
...
Lo que ese camino NO puede resolver son dos casos, y los dos son de listado:
