...
- `src/components/head/AnalyticsScripts.astro` — **tampoco existe**, y nunca
  ...
  si fuera un plan vivo. Ese roadmap es del sitio viejo entero; no se sigue.
...
**Esto no se «arregla» poniendo los ids en el `.env`.** Las cinco variables
...
**`src/js/analytics.js` es LEGADO y no se toca.** Existe en el repo, pero
...
- **`ga4()` nunca puede tumbar la reproducción**, y de eso se encarga su
...
- **Un contenedor en el `<head>` de `Base.astro` se ejecuta UNA vez**, porque el
...
- **comScore es el que más pesa comercialmente**, aunque sea el más pequeño: es
...
**Y lo primero que hay que resolver es una pregunta, no un `<script>`:** si GA4
...
- **GPT, los huecos y el conteo de la venta directa** — son de `agents/ads.md`. Se
...
**El diagnóstico del player ya está hecho y sirve para esto**: `?depurar=player`
...
Y se verifica **contra el HTML servido**, con `curl`, no abriendo la página: un
