...
| `src/components/IndiceScanner.astro` | El interior compartido de `/beat-scanner`, `/editorial`, `/beat-scanner/*` y `/etiqueta/*`. `/microambiente` NO lo usa. Desde el 2026-09-17 pagina: la destacada es de la página 1 y de la 2 en adelante la cuadrícula es lo primero (`.es-cabeza`) |
...
| `src/components/AudioNota.astro` | El audio principal de una nota, pegado bajo la foto. El COMPORTAMIENTO es de `agents/streaming.md` |
| `src/components/Paginador.astro` | La tira de páginas de los índices. Es SERVIDOR, no JavaScript, y con una sola página no pinta nada |
...
**`src/js/` entero es LEGADO** y no se toca: comprobado con `grep`, ningún archivo
...
**Y por eso `!important` casi nunca es la respuesta aquí**: el síntoma de «no me
...
1. **Los `@import` van CON COMILLAS**, no con `url()`. Tailwind 4 procesa el CSS con
   ...
   descarta al empaquetar. Resultado medido: los nueve archivos de tokens no se
...
Y `tailwind.config.js` tiene su propia regla: **ni un valor literal.** Solo mapea
...
- **El sitio va en BLANCO Y NEGRO** (Carlos, 2026-08-27). El DS trae una paleta
  ...
  hubo una decisión. Los tokens `--luz-*` NO se tocan — siguen sirviendo en
...
- **En los INTERIORES la barra oscura nace recogida** (Carlos, 2026-09-01), y en
  ...
  día se olvida. El plegado móvil va en **CSS y no solo en el script**, porque el
...
- **Astro NO admite comentarios de llaves dentro de la lista de atributos.** Los
...
- **`astro check` no basta como puerta.** Uno de esos casos daba **0 errores** en
  `pnpm check` y solo tronó el compilador del build. La puerta es `pnpm build`.
- **Un atributo booleano se pasa como booleano**: `inert={compacta}`, no
...
- **Tailwind 4 quitó `cursor: pointer` de su preflight.** Un `<button>` se comporta
  como texto: el puntero no cambia y el control no se anuncia como pulsable. Hay que
  ponerlo a mano.
- **`transition:persist` necesita un NOMBRE explícito.** Sin nombre, Astro genera
...
**Y `prefers-reduced-motion` no es opcional.** El parpadeo del punto de aire va
...
- **Los huecos de publicidad**: `src/components/Anuncio.astro`, sus medidas y su
  colocación son de `agents/ads.md`. Y el marco se reserva y luego DESAPARECE: eso
...
- **El player**: `src/scripts/player.ts`, `audio.ts`, `pista.ts`, `senal.ts`,
  `video.ts` y `audio-nota.ts` son de `agents/streaming.md`. Se cruzan en
...
- **Decidir la maqueta de una pantalla que el lienzo no describe.** El lienzo cubre el
  Inicio (13a/14a), Scanner (14b/14e) y la nota (14c/14f). `/en-vivo` sigue sin
...
**Y el panel del navegador MIENTE sobre las animaciones.** Una animación que ya
...
**El contraste se MIDE, no se estima** (`movimiento.md` §4): sobre un fondo oscuro
el ojo perdona lo que la norma no.

Y en móvil se comprueba **a 375px**, no encogiendo la ventana a ojo. Ahí se
