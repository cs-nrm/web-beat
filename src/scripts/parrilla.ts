 ...
 *
 * El nombre del archivo se quedó viejo: el hero vivía en `/programacion` hasta
 ...
 * `movimiento.md`.
 *
 * Al lado del botón vivía «Ver el show», un enlace a la ficha del programa. Se
 * retiró el 2026-09-09 (Carlos: «quitar el botón ver show») y no dejó rastro aquí:
 * era un `<a>`, nunca pasó por este archivo.
 *
 * Aquí vivían además las tres casillas que resaltaban un tipo de show en la
 ...
 * tiene.
 *
 * Y lo que la parrilla sí necesita —que las celdas y el punto de tally se
 ...
 *
 * De los cinco estados del player (`init`, `cargando`, `sonando`, `pausa`,
...
function copiarEstado(): void {
  ...
  /*
    ...
    Y el hero se mudó de página sin que esto cambiara, que es justo la gracia
  ...
  */
  ...
  /*
    Se hereda el `disabled` del original, y no es un detalle cosmético: el botón
...
export function prepararParrilla(): void {
  ...
  */
   ...
   *
   * `#player` vive en la cabecera, que lleva `transition:persist`, así que el
